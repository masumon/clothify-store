...
const OrderSchema = z.object({
  ...
  delivery_method: z
    ...
    .transform((value) => (value === "Store Pickup" ? "Pickup" : value)),
...
export type OrderRouteDeps = {
  getSupabaseClient?: typeof getSupabaseAdminClient;
  sendNotification?: typeof sendWhatsAppNotification;
...
async function generateInvoiceNumber(
  ...
  let serialBase = Number(count || 0) + 1;
  const createdAtError = (error?.message || "").toLowerCase();
  if (createdAtError.includes("created_at")) {
    const fallback = await supabase
      .from("orders")
      .select("id", { count: "exact", head: true });
    serialBase = Number(fallback.count || 0) + 1;
  }

  const serial = padSerial(serialBase, 4);
  return `INV-${dateKey}-${serial}`;
}

async function defaultOrderRateLimit(request: Request) {
...
export async function handleCreateOrder(
  ...
  const getSupabaseClient = deps.getSupabaseClient || getSupabaseAdminClient;
  const sendNotification = deps.sendNotification || sendWhatsAppNotification;
  const consumeRateLimit = deps.consumeRateLimit || defaultOrderRateLimit;
  ...
  try {
    ...
    const finalTrxId =
      payment_method === "Cash on Delivery"
        ? `COD-${now().toString(36).toUpperCase()}`
        : normalizedTrx;
    ...
    if (typeof client_started_at === "number") {
      const elapsed = now() - client_started_at;
    ...
    if (insertedOrder.error) {
      return NextResponse.json({ error: insertedOrder.error.message }, { status: 500 });
    }

    const orderId = insertedOrder.data?.id;
    let createdAtIso = new Date(now()).toISOString();

    if (orderId) {
      const createdAtLookup = await supabase
        .from("orders")
        .select("created_at")
        .eq("id", orderId)
        .single();
      if (!createdAtLookup.error) {
        const lookedUp =
          (createdAtLookup.data as { created_at?: string | null } | null)?.created_at || "";
        if (typeof lookedUp === "string" && lookedUp) {
          createdAtIso = lookedUp;
        }
      }
    }
    ...
    let invoiceNumber = "";
    if (data.id) {
      invoiceNumber = await generateInvoiceNumber(supabase, createdAtIso);
    }
